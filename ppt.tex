...
\begin{frame}{Dominant Shift: Gravity Axis Projection}
...
\begin{itemize}
    \item Waist mount: gravity falls along \textbf{X-axis}\\
    Mean $= +8.58$\,m/s$^2$
    \item Front pocket: gravity falls along \textbf{Y-axis}\\
    Mean $= +6.51$\,m/s$^2$
    \item X-axis Wasserstein $= \mathbf{8.7}$\,m/s$^2$
\end{itemize}

\bigskip
This is not a subtle shift. \\
The sensor is physically rotated $\sim 90^\circ$.

\bigskip
A model trained on UCI HAR learns that ``X has a big DC component.'' In WISDM, X is nearly zero-mean.
\end{column}
\end{columns}
...
\begin{center}
\includegraphics[width=0.82\textwidth]{figures/latent_space_overlap.png}
\end{center}

\medskip
\textbf{Multi-kernel MMD = 0.830} (before any adaptation)

\smallskip
The two datasets barely overlap in feature space.\\
A classifier trained on UCI HAR has no chance on WISDM as-is.
...
\begin{center}
\includegraphics[width=0.82\textwidth]{figures/distribution_alignment.png}
\end{center}

\medskip
\textbf{MMD: $0.830 \rightarrow 0.453$} \quad (45.4\% reduction)

\smallskip
The gap is substantially smaller, but not gone.\\
The remaining gap likely explains why some classes are still hard.
...
\includegraphics[width=\textwidth]{figures/radar_charts.png}
\end{column}
...
\begin{frame}{Why TTBN and DANN Don't Help}
...
\begin{itemize}
    ...
    \item PhysicalDistortion: \textbf{F1 = 0.723} ($6\times$ improvement)
